...
public class FileScanFailedConsumer {

    private static final String EVENT_IDEMPOTENT_PREFIX = "quota:event:scan_failed:";
    ...
    public void handleFileScanFailed(FileScanFailedEvent event,
        ...
        try {
            ...
            if (Boolean.FALSE.equals(success)) {
                log.warn("文件扫毒失败事件已处理（幂等跳过）: eventId={}", event.getEventId());
            ...
            log.error("文件扫毒失败事件处理失败: eventId={}, error={}", event.getEventId(), e.getMessage(), e);
            try {
