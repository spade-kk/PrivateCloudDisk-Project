...
public class GroupController {
    ...
    public Result<GroupDTO> createGroup(
            @Parameter(description = "群主 ID") @RequestParam String ownerId,
            @Parameter(description = "群组名称") @RequestParam String groupName,
    ...
    public Result<GroupDTO> getGroupDetail(
            @Parameter(description = "群组 ID") @PathVariable String groupId) {
    ...
    public Result<Void> kickMember(
            @Parameter(description = "群组 ID") @PathVariable String groupId,
            @Parameter(description = "操作者 ID") @RequestParam String operatorId,
            @Parameter(description = "被踢用户 ID") @RequestParam String targetUid) {
    ...
    public Result<Void> muteMember(
            @Parameter(description = "群组 ID") @PathVariable String groupId,
            @Parameter(description = "操作者 ID") @RequestParam String operatorId,
            @Parameter(description = "被禁言用户 ID") @RequestParam String targetUid,
            @Parameter(description = "禁言时长（分钟，-1 表示永久）") @RequestParam int durationMinutes) {
    ...
    public Result<Void> unmuteMember(
            @Parameter(description = "群组 ID") @PathVariable String groupId,
            @Parameter(description = "操作者 ID") @RequestParam String operatorId,
            @Parameter(description = "被解除禁言用户 ID") @RequestParam String targetUid) {
    ...
    public Result<Void> muteAll(
            @Parameter(description = "群组 ID") @PathVariable String groupId,
            @Parameter(description = "操作者 ID（群主）") @RequestParam String operatorId,
