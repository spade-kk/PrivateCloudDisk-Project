//! CloudFlow Capability Agent（独立 crate `cloudflow-agent`）。
//!
//! 分层定位（执行引擎统一架构 · 需求 §七/§八）：
//! - 本 crate 是**唯一能力调用出口**：执行引擎核心（`cloudflow-engine-core`）
//!   不直接访问数据库、文件系统或其他微服务，全部动作经 `CapabilityInvoker`
//!   抽象下发给 Agent（gRPC）；
//! - 生产执行面（宿主 crate `pcd-cloudflow-runtime` 的 `execution` 模块）
//!   经 `GrpcCapabilityInvoker` 连接本服务；`CapabilityAgentProxy` 作为 gRPC
//!   服务端把受控调用转交 Workflow Service Capability Hub 内部接口；
//! - CLI（`cloudflowc`）与默认调试面不依赖本 crate（Mock 执行器替代）。
//!
...
use cloudflow_engine_core::ir::ActionIr;
