...
//!
//! 本模块是**纯内存 DAG 状态机**：统一驱动（`crate::engine::driver`）用它做
//! 就绪节点计算与完成簿记；具体 Capability/Plugin/API 适配器由 `EngineDeps`
//! 注入的动作执行器承担，本模块不直接执行用户代码。
...
impl RuntimeEngine {
    pub fn load(execution_id: impl Into<String>, ir: WorkflowIrV1) -> Result<Self, Vec<String>> {
        // IR 契约校验唯一实现在 `crate::ir_validate`；文本格式与宿主 crate
        // `compiler::validate_ir` 适配层完全一致（code: message (path)）。
        let errors = crate::ir_validate::validate_ir_contracts(&ir)
            .into_iter()
        ...
        Ok(Self::load_unvalidated(execution_id, ir))
    }

    /// 不做 IR 契约校验直接构造引擎；仅供 dev/debug 入口在 `skip_validation` 时
    /// 测试校验器本身使用。生产执行路径必须走 `load`（保持契约校验强制）。
    pub fn load_unvalidated(execution_id: impl Into<String>, ir: WorkflowIrV1) -> Self {
        let task_states = ir
            .spec
            .graph
            .nodes
