...
class PreviewResourceRepository:
    ...
    async def upsert(self, resource: PreviewResource) -> str:
        ...
        sql = """
            INSERT INTO pcd_preview_resource_table (
                resource_id, file_id, user_id, resource_type, resource_variant,
                ...
                UUID_TO_BIN(COALESCE(%s, UUID())), UUID_TO_BIN(%s), UUID_TO_BIN(%s), %s, %s,
        ...
        """
        values = (
            resource.resource_id, resource.file_id, resource.user_id, resource.resource_type,
    ...
    async def list_by_file(self, file_id: str, user_id: str, include_deleted: bool = False) -> list[dict[str, Any]]:
        pool = await get_database_pool()
        ...
        sql = f"""
            SELECT BIN_TO_UUID(resource_id) resource_id, BIN_TO_UUID(file_id) file_id,
                   BIN_TO_UUID(user_id) user_id, resource_type, resource_variant,
            ...
            WHERE file_id=UUID_TO_BIN(%s) AND user_id=UUID_TO_BIN(%s) {deleted_clause}
            ORDER BY resource_type, resource_variant
        """
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute(sql, (file_id, user_id))
                rows = await cursor.fetchall()
        return [self._normalize(row) for row in rows]

    async def get_ready(self, file_id: str, user_id: str, resource_type: str, variant: str = "default") -> dict[str, Any] | None:
        resources = await self.list_by_file(file_id, user_id)
    ...
    async def mark_file_deleted(self, file_id: str, user_id: str) -> None:
        pool = await get_database_pool()
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute(
                    "UPDATE pcd_preview_resource_table SET resource_status='deleted', deleted_at=CURRENT_TIMESTAMP(3) "
                    "WHERE file_id=UUID_TO_BIN(%s) AND user_id=UUID_TO_BIN(%s)",
                    (file_id, user_id),
                )
            await conn.commit()

    async def hard_delete_file_records(self, file_id: str, user_id: str) -> None:
        """
    ...
    async def count_ready_videos(self, user_id: str) -> int:
        pool = await get_database_pool()
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                await cursor.execute(
                    "SELECT COUNT(DISTINCT file_id) total FROM pcd_preview_resource_table "
                    "WHERE user_id=UUID_TO_BIN(%s) AND resource_type='hls' AND resource_status='ready'",
                    (user_id,),
                )
                row = await cursor.fetchone()
