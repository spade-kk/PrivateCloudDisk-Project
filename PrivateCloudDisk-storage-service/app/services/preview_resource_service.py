...
class PreviewResourceService:
    def _cache_key(self, user_id: str, file_id: str) -> str:
        return f"preview:resources:{user_id}:{file_id}"

    async def list_resources(self, file_id: str, user_id: str) -> list[dict[str, Any]]:
        key = self._cache_key(user_id, file_id)
        cached = None
        try:
        ...
        resources = await preview_resource_repository.list_by_file(file_id, user_id)
        try:
    ...
    async def get_ready(self, file_id: str, user_id: str, resource_type: str, variant: str = "default") -> dict[str, Any] | None:
        resources = await self.list_resources(file_id, user_id)
    ...
    async def upsert(self, resource: PreviewResource) -> str:
        ...
        try:
            await redis_client.delete(self._cache_key(resource.user_id, resource.file_id))
        except Exception as exc:
            logger.warning("预览资源已持久化，但 Redis 缓存失效失败: %s", exc)
        return resource_id

    async def delete_file_resources(self, file_id: str, user_id: str) -> list[str]:
        ...
        await preview_resource_repository.hard_delete_file_records(file_id, user_id)
        try:
