...
export const useSearchStore = defineStore('search', () => {
  ...
  const keyword = ref('')
  const filters = ref<Record<string, string>>({ ...DEFAULT_FILTERS })
  ...
  const recentKeywords = ref<string[]>(JSON.parse(localStorage.getItem('cloudDriveSearchRecent') || '[]'))

  const totalPages = computed(() => Math.max(1, Math.ceil(total.value / size.value)))
  const hasQuery = computed(() => keyword.value.trim() || Object.values(filters.value).some(Boolean))

  function persistRecent(term: string): void {
  ...
  function setFilters(nextFilters: Record<string, string> = {}): void {
  ...
  function buildRequest(overrides: Record<string, any> = {}): Record<string, any> {
  ...
  async function search(overrides: Record<string, any> = {}): Promise<{ success: boolean; message?: string }> {
    loading.value = true
    error.value = null
  ...
  async function goToPage(nextPage: number): Promise<{ success: boolean; message?: string }> {
